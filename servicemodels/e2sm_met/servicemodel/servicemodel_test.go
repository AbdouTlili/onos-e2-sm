...
func TestServicemodel_IndicationHeaderProtoToASN1(t *testing.T) {
	...
	globalMetNodeID, err := pdubuilder.CreateGlobalMetnodeIDgNBID(&bs, plmnID)
	globalMetNodeID.GetGNb().GNbCuUpId = &e2smmet.GnbCuUpId{
		Value: gnbCuUpID,
	}
	globalMetNodeID.GetGNb().GNbDuId = &e2smmet.GnbDuId{
		Value: gnbDuID,
	}
	...
	assert.NilError(t, err)
	newE2SmMetPdu.SetFileFormatVersion(fileFormatVersion).SetSenderName(senderName).SetSenderType(senderType).SetVendorName(vendorName).SetGlobalMETnodeID(globalMetNodeID)
	assert.NilError(t, err)
...
func TestServicemodel_IndicationMessageProtoToASN1(t *testing.T) {
	...
	var distY int32 = 456
	var distZ int32 = 789
	startEndIndication := e2smmet.StartEndInd_START_END_IND_START
	var measurementName = "trial"
	sum := e2smmet.SUM_SUM_TRUE
	plo := e2smmet.PreLabelOverride_PRE_LABEL_OVERRIDE_TRUE
	...
	labelInfoList := e2smmet.LabelInfoList{
		Value: make([]*e2smmet.LabelInfoItem, 0),
	}
...
func TestServicemodel_RanFuncDescriptionProtoToASN1(t *testing.T) {
	...
	cmol := make([]*e2smmet.CellMeasurementObjectItem, 0)
	cmol = append(cmol, cellMeasObjItem)

	metNodeItem := pdubuilder.CreateRicMetnodeItem(globalMetnodeID).SetCellMeasurementObjectList(cmol)

	rknl := make([]*e2smmet.RicMetnodeItem, 0)
	rknl = append(rknl, metNodeItem)
	...
	assert.Assert(t, newE2SmMetPdu != nil)
	newE2SmMetPdu.SetRanFunctionInstance(rfi).SetRicMetNodeList(rknl).SetRicReportStyleList(rrsl).SetRicEventTriggerStyleList(retsl)
